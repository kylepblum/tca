function info = viz_ktensor_td(K, td, varargin)
...
params.addParameter('SplitBy', 'None');

params.parse(varargin{:});
res = params.Results;
%%
if ~strcmp(res.SplitBy, 'None')
    numSplits = unique([td.(res.SplitBy)]);
    numSplits(isnan(numSplits)) = [];
    tdSplit = [td.(res.SplitBy)];
    tdSplit(isnan(tdSplit)) = [];
    colorSpec = linspecer(length(numSplits));
    for i =1:length(tdSplit)
        res.Plotcolors{3}(i,:) = colorSpec(numSplits == tdSplit(i),:);
    end
end
...
%                 hh = scatter(FactorAxes(k,j), xx, yy, ss, cc, 'filled');
                hh = plot(FactorAxes(k,j), xx, smooth(yy));
            case 'bar'
                if k == 1 && j == 1
                    [~,sortOrder] = sort(yy);
                end

                hh = bar(FactorAxes(k,j), xx, yy(sortOrder), 'EdgeColor', cc, 'FaceColor', cc);
        end
